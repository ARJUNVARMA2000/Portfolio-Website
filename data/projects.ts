...
const classPulseCaseStudy: CaseStudy = {
  problem: `In live classrooms, professors often ask open-ended questions to gauge student understanding, but collecting and synthesizing dozens of handwritten or verbal responses in real time is impractical. Students may hesitate to speak up, and manual aggregation of feedback misses emerging themes.

The goal was to create a tool that lets professors pose a question, collect student responses instantly via QR code or link, and automatically summarize them into key themes with attribution—turning messy classroom input into actionable insights during the session.`,

  challenge: `Building a real-time classroom feedback system presented several technical hurdles:

Key challenges:
• Need to aggregate and process submissions as they arrive without blocking the UI
...
• QR code and shareable link flow must be frictionless for students on any device`,
  ...
  approach: `The system was designed with a clear professor–student–AI flow:

1. Session Creation: Professor creates a session with a question; system generates a unique QR code and shareable link

2. Student Submission: Students open the link (no signup), type their answer, and submit—responses are stored and associated with the session

3. Periodic Summarization: Backend runs a summarization job every 10 seconds when 3+ responses exist, using OpenRouter with a 5-model fallback chain

4. Real-Time Updates: FastAPI serves Server-Sent Events (SSE) so the professor's dashboard updates automatically as new themes are generated`,

  solution: `The production architecture includes:

• FastAPI Backend: REST API for sessions, responses, and theme summaries; SSE endpoint for real-time dashboard updates
• React/Vite Frontend: Professor dashboard with theme cards and student attribution; student submission form with QR code display
• OpenRouter Integration: 5-model fallback chain (Gemini 2.0 Flash, Llama 3.1, Mistral 7B, Gemma 2, Qwen 2.5) for reliable summarization
• Single Dockerfile: Builds React static assets, then runs FastAPI serving both API and static files
• Railway Deployment: One service with auto-deploy on push; OpenRouter API key stored as Railway variable`,

  impact: `• Enables live theme extraction from student responses in real time during class
...
export const projects: ProjectData[] = [
  ...
  {
    title: "ClassPulse",
    subtitle: "Live Classroom Theme Extraction",
    description:
